...
//** Purebas en el archivo registro */
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    if (isset($_POST['iniciar'])){
        if (strlen($tip) >= 1 && strlen($con) >= 1 && strlen($nom) >= 1 && strlen($ape) >= 1 && strlen($cor) >= 1 && strlen($tel) >= 1 &&  strlen($nac) >= 1 ){
            ...
            $consulta = "INSERT INTO usuarios(tpusuarioid, contraseña, nombres, apellidos, correos, telefonos, nacimientos) VALUES ('$tipo', '$contra', '$nombres', '$apellidos', '$correos', '$telefonos', '$nacimientos')";
            $resultado = mysqli_query($conex,$consulta);
            if ($resultado){
                ?>
...
?>
            <br>
            <a href="profregistro.html" class="buttotro">Registrar otro profesor</a>
            <br>
            <br>
            <br>
